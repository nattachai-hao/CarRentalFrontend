...
export default function CarPanel() {
    ...
    const [ compairList, dispatchCompare ] = useReducer(compareReducer,new Set<string>())
    ...
    return (
        <div>
            ...
            <div className="w-full text-xl font-medium">Compare List {compairList.size}</div>
            { Array.from(compairList).map( (car) => <div key={car} onClick={() => dispatchCompare({type:"remove",carName:car})}
            >{car}</div>) }
            <button className="block rounded-md bg-sky-600 hover:bg-indigo-600 !px-3 !py-2 text-white shadow-small"
